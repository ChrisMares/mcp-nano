...
async fn server_and_tool_crud() {
    ...
    let created = mcp_config::create_server(&pool, "My Server".into(), Some("desc".into()))
        .await
        .expect("create server");
    assert_eq!(created.server.name, "My Server");
    ...
    let active = mcp_config::list_active_tools(&pool, Some(&server_id))
        .await
        .expect("active tools");
    ...
    let active2 = mcp_config::list_active_tools(&pool, Some(&server_id))
        .await
    ...
    assert!(info.full_url.contains("server_id="));
    assert!(info.full_url.contains("18651"));
...
async fn find_tool_by_name_respects_server_id() {
    ...
    let t1 = mcp_config::find_active_tool_by_name(&pool, Some(&s1.server.id), "shared_name")
        .await
        .unwrap()
        .expect("tool on s1");
    assert_eq!(t1.description.as_deref(), Some("a"));

    let t2 = mcp_config::find_active_tool_by_name(&pool, Some(&s2.server.id), "shared_name")
        .await
        .unwrap()
        .expect("tool on s2");
